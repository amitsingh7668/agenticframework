...
DB_URL = os.getenv("POSTGRES_DB_URL", "postgresql://agentuser:secret123@localhost:5432/agents_db")
...
def clear_expired_sessions(timeout_minutes=30):
    with get_connection() as conn, conn.cursor() as cur:
        ...
        cur.execute(f"""
            ...
            USING last_activity
            WHERE messages.session_id = last_activity.session_id
            AND last_seen < NOW() - INTERVAL '{timeout_minutes} minutes';
        """)
        print(f"[✓] Expired sessions older than {timeout_minutes} minutes cleared.")
