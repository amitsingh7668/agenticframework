...
class SalesOrchestrator:
    ...
    async def process_query(
        ...
        try:
            ...
            print(f"👤 User context: {user_context if user_context else 'No context provided'}")
            context = self._create_sales_context(user_context)
            print(context)

            print("📂 Using session for conversation history" if session else "📂 No session provided, starting fresh")
            ...
            print(f"✅ Query processed successfully in {end_time - start_time:.2f} seconds response: {result}")
    ...
    def _create_sales_context(self, user_context: Optional[Dict[str, Any]] = None) -> SalesContext:
        ...
        print(f"👤 Creating sales context with user info: {user_context}")
        return SalesContext(
